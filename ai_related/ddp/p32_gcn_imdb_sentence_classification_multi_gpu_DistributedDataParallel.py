"""
    文本分类GPU版本：单机多卡——torch.nn.DistributedDataPatallel（多进程，效率高，多机多卡也是用这个方法）
    运行命令：
    1. python -m torch.distributed.launch --nproc_per_node=2 p32_gcn_imdb_sentence_classification_multi_gpu_DistributedDataParallel.py
    2. torchrun --nproc_per_node=2 p32_gcn_imdb_sentence_classification_multi_gpu_DistributedDataParallel.py
    不知道为啥，第一个会报错。第二个跑不起来。
"""
import torch
...
class TextClassificationModel(nn.Module):
    ...
    def forward(self, token_index):
        embedded = self.embedding(token_index)      # [bs, embedding_dim]
...
# 修改1####################################
BATCH_SIZE = 64 * 2     # 用两张卡就*2，每张卡就都是64大小#########################
...
def collate_fn(batch):
    ...
    return (torch.tensor(target).to(torch.int64), torch.tensor(token_index).to(torch.int64))


# 编写训练代码
def train(local_rank, train_dataset, eval_dataset, model, optimizer, num_epoch, log_step_interval, 
          save_step_interval, eval_step_interval, save_path, resume=""):
    """此处的data_loader的map-style dataset"""
    ...
    for epoch_index in range(start_epoch, num_epoch):
        ...
        for batch_index, (target, token_index) in enumerate(train_data_loader):
            ...
            if step % save_step_interval == 0 and local_rank == 0:      # 只在GPU0的时候进行保存########################
                os.makedirs(save_path, exist_ok=True)
                save_file = os.path.join(save_path, f"step_{step}.pt")
                torch.save({'epoch': epoch_index, 
                            'step': step, 
                            'model_state_dict': model.module.state_dict(),      # 因为包裹在DataParallel里##########################
                            'loss': bce_loss,
                            }, save_file)
            ...
            if step % eval_step_interval == 0:
                ...
                for eval_batch_index, (eval_target, eval_token_index) in enumerate(eval_data_loader):
                    ...
                    eval_token_index = eval_token_index.cuda(local_rank)      # 数据拷贝###############################
                    eval_target = eval_target.cuda(local_rank)                # 数据拷贝###############################
...
def main():
    ...
    torch.cuda.set_device(args.local_rank)      # 相当于os.environ["CUDA_VISIBLE_DEVICE"]
