...
class DQNAgent(object):
    ...
    def learn_batch(
        self, batch_obs, batch_action, batch_reward, batch_next_obs, batch_done
    ):
        # predict_Q
        pred_Vs = self.preq_func(batch_obs)
    ...
    def learn(self, obs, action, reward, next_obs, done):
        ...
        if (
            len(self.rb) > self.replay_start_size
            and self.global_step % self.rb.num_steps == 0
        ):
    ...
    def sync_target(self):
        for target_param, param in zip(self.target_func.parameters(), self.preq_func.parameters()):
