...
dataset = torchvision.datasets.MNIST('data/GAN_MNIST',
                                     train=True,
                                     transform=transform,
...
class Generator(nn.Module):
    ...
    def forward(self, x1, x2):      # x1:label, x2:noise
...
class Discriminator(nn.Module):
    ...
    def forward(self, x1, x2):          # x1:label, x2:image
...
label_seed_onehot = one_hot(label_seed).to(device)          # 在测试过程中随机给的一个label, 就跟训练过程中加载了真的label一个道理
print(label_seed)
